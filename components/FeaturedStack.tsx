    ...
    <div className="min-h-screen p-4">
      ...
      <div className="flex justify-center mt-24">
        <div className="flex rounded-md bg-red-200 w-32 h-8 justify-center text-center items-center gap-2 p-2">
          <Icon icon="ph:user-circle-check-light" className="w-4 h-4 text-[#5b1219] font-bold" />
      ...
      <div className="flex flex-col mb-4">
        ...
        <p className="text-center mt-4 text-gray-500 font-medium">
          ...
          cutting-edge technologies to ensure seamless <br /> and dynamic designs.
        </p>
      </div>
      <div className="grid grid-cols-2 max-w-[60%] gap-4 mt-4 mx-auto">
        {tools.map((tool) => (
            <Card key={tool.id} icon={tool.icon} title={tool.title} description={tool.description} />
        ))}
      </div>
    </div>
  );
